...
train_aware_path = os.path.join('train_aware.json')
train_agnostic_path = os.path.join('train_agnostic.json')
dev_aware_path = os.path.join('dev_aware.json')
dev_agnostic_path = os.path.join('dev_agnostic.json')
...
def get_train_data(batch_size=8, max_length=128, overfit=False, remove_unnecessary_cols=False):
    df_agnostic = pd.read_json(train_agnostic_path)
    df_aware = pd.read_json(train_aware_path)
...
def get_dev_data(batch_size=8, max_length=128, overfit=False):
    df_agnostic = pd.read_json(dev_agnostic_path)
    df_aware = pd.read_json(dev_aware_path)
